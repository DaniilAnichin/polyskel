...
from euclid import Point2, Ray2, Line2, LineSegment2
...
def _normalize_contour(contour):
    ...
    return [point for _prev, point, _next in _window(contour) if not (
            point == _next or (point - _prev).normalized() == (_next - point).normalized())]
...
class _LAVertex:
    def __init__(self, point, edge_left, edge_right, direction_vectors=None):
        ...
        _debug.line((self.bisector.p.x, self.bisector.p.y,
                     self.bisector.p.x + self.bisector.v.x * 100, self.bisector.p.y + self.bisector.v.y*100),
                    fill="blue")
    ...
    def __repr__(self):
        return "Vertex ({}) ({:.2f};{:.2f}), bisector {}, edges {} {}".format(
            ...
            self.point.x, self.point.y, self.bisector, self.edge_left, self.edge_right)
    ...
    def handle_edge_event(self, event):
        ...
        if event.vertex_a.prev == event.vertex_b.next:
            log.info("%.2f Peak event at intersection %s from <%s,%s,%s> in %s",
                     event.distance, event.intersection_point, event.vertex_a, event.vertex_b, event.vertex_a.prev, lav)
...
class _LAV:
    ...
    def from_polygon(cls, polygon, slav):
        lav = cls(slav)
        for prev, point, _next in _window(polygon):
            lav._len += 1
            vertex = _LAVertex(point, LineSegment2(prev, point), LineSegment2(point, _next))
            vertex.lav = lav
            if lav.head is None:
                lav.head = vertex
...
def skeletonize(polygon, holes=None):
    ...
    while not (prior_queue.empty() or slav.empty()):
        ...
        if isinstance(i, _EdgeEvent):
            ...
            raise ValueError('Event of unworkable type caught')

        prior_queue.put_all(events)
